...
void ShellSort(int a[]){
     ...
     for(gap=n/2;gap>0;gap /= 2) {
		  //遍历所有的数（除第一组？）
          for(i=gap;i<n;i++) {
			   //这里确实是插入排序（将一个未排序的数字，插入到前面已经排序的序列）
...
void Merge(int sr[],int tr[],int i,int m,int l){
	...
	// //首先考虑下如何将将二个有序数列合并。这个非常简单，只要从比较二个数列的第一个数，谁小就先取谁，取了后就在对应数列中删除这个数。然后再进行比较，如果有数列为空，那直/////接将另一个数列的数据依次取出即可。
...
void Sort(int a[]){
	...
	//printf(  "双向冒泡排序: 比较次数为:%5ld 移动次数为:%6ld 基本操作次数为：%ld\n",com[7],exc[7],com[7]+exc[7]);
